...
#
# Usage: update-blacklist.sh [OPTIONS] <config-file>
#
# Options:
...
# Temporary files (registered via make_temp, cleaned up on exit)
...
# Create a temp file, register it for cleanup, and store its path in the
# variable named by $1.
#
# IMPORTANT: this must NOT be called as `x=$(make_temp)`. Command
# substitution runs the function in a subshell, so the TEMP_FILES+=(...)
# below would only mutate a copy of the array that is discarded when the
# subshell exits -- the path would still be printed correctly, but the file
# would never actually be registered for cleanup, leaking tmp.* files in
# /tmp on every run. Using a nameref output parameter keeps everything in
# the current shell so the registration actually sticks.
#
# Usage: make_temp my_var   (afterwards: $my_var holds the new temp path)
make_temp() {
    local -n __make_temp_out="$1"
    __make_temp_out=$(mktemp)
    TEMP_FILES+=("$__make_temp_out")
}
...
# Output chunked "add element" statements for a set
...
download_blacklist() {
    ...
    case "$http_code" in
        200|301|302|000)
            ...
            # 000     = file:// URL
            # Guard against a "successful" transfer that produced no data
            # (e.g. connection reset with no HTTP status, empty file:// path).
...
main() {
    # Set up cleanup trap as early as possible so that any temp file created
    # during argument parsing or later is guaranteed to be removed on exit,
    # regardless of future code changes to this function.
    trap cleanup EXIT
    ...
    make_temp ipv4_raw
    make_temp ipv6_raw
    make_temp ipv4_clean
    make_temp ipv6_clean
        ...
        if [[ -s "$ipv4_raw" ]]; then
            ...
            if [[ -s "$ipv4_clean" ]]; then
                ...
                if iprange --optimize "$ipv4_clean" > "$ipv4_optimized" 2>/dev/null && [[ -s "$ipv4_optimized" ]]; then
                    ...
                    log_info "  CIDR optimization: $before_count -> $after_count entries ($((before_count - after_count)) merged)"
                else
                    log_warn "CIDR optimization skipped (iprange --optimize failed); using unoptimized list"
                fi
            fi
        fi
    fi
    ...
    make_temp whitelist_v4
    make_temp whitelist_v6
    ...
    if [[ "${AUTO_WHITELIST:-no}" == "yes" ]]; then
        ...
        if [[ -s "$auto_ips" ]]; then
            ...
            log_warn "Auto-whitelist enabled but no server IPs could be detected"
        fi
    fi
